...
def power_compress(x):
    """Compresses the power of a complex spectrogram using power compression.

    This function takes a complex spectrogram represented as separate real and imaginary
    components and applies power compression to the magnitude while preserving the phase.
    The compression is achieved by raising the magnitude to the power of 0.3.
    ...
    # NPU-compatible magnitude calculation (avoid complex operations)
    mag = torch.sqrt(real**2 + imag**2)  # Compute magnitude directly
    phase = torch.atan2(imag, real)  # Compute phase directly
