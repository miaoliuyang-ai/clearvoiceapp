...
def save_audio_debug_data(audio_data: bytes, audio_array: np.ndarray, session_id: str, data_type: str, timestamp: str):
    """保存音频数据用于调试（仅保存WAV文件）

    Args:
        audio_data: 原始音频二进制数据（用于日志记录大小）
        audio_array: 处理后的音频数组
        session_id: 会话ID
        data_type: 数据类型 ('original', 'denoised', 'failed', 'error')
        timestamp: 时间戳字符串
    """
    try:
        # 创建debug_audio目录
        ...
        if audio_array is not None:
            try:
                ...
                wav_filename = f"{timestamp}_{data_type}.wav"
                wav_filepath = os.path.join(session_dir, wav_filename)
                wavfile.write(wav_filepath, sample_rate, audio_int16)
                logger.info(f"[{session_id}] 保存{data_type}WAV文件: {wav_filepath} (大小: {len(audio_data)} 字节)")

            except Exception as e:
                logger.warning(f"[{session_id}] 保存{data_type}WAV文件失败: {e}")
        else:
            logger.warning(f"[{session_id}] 音频数组为空，跳过{data_type}WAV文件保存")

    except Exception as e:
        logger.error(f"[{session_id}] 保存{data_type}音频数据失败: {e}")
    ...
    try:
        ...
        if os.path.exists(sample_file_path):
            ...
            try:
                ...
                clearvoice_instance(sample_file_path, False)
...
async def process_audio(
    ...
    try:
        ...
        if denoised_audio is not None:
            ...
            # 保存降噪后的音频数据用于调试
            save_audio_debug_data(processed_audio_bytes, denoised_audio, session_id, "denoised", timestamp)
